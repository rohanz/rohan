...
async function syncProjectContent(drawingSlugs) {
  ...
  // Both registries come from this same complete list, including unlisted work.
  const imports = projectData.map(({ slug }, i) =>
    `import article${i} from './content/articles/${slug}.md?raw';`).join('\n');
  const entries = projectData.map(({ slug }, i) => `  ${JSON.stringify(slug)}: article${i},`).join('\n');
  await writeFile(path.join(blueprintRoot, 'src', 'articles.generated.js'),
...
var phone = matchMedia('(pointer: coarse)').matches && Math.min(screen.width, innerWidth || 9e9) < 1024 && !params.has('desktop');
params.set('p', location.pathname);
location.replace(phone ? '${canonical}' + location.search + location.hash : '/blueprint/?' + params + location.hash);
</script>
</head>
<body></body>
</html>
...
// Generation is deliberately independent of the sub-app's dependency tree.
...
async function ensureBlueprintDependencies() {
  const lock = await readFile(path.join(blueprintRoot, 'package-lock.json'));
  const hash = createHash('sha256').update(lock).digest('hex');
  const marker = path.join(blueprintRoot, 'node_modules', '.blueprint-lock.sha256');
  let installedHash;
  try { installedHash = await readFile(marker, 'utf8'); } catch { /* first build */ }
  // The hidden npm lock omits other platforms' optional packages, so comparing
  // its entire JSON would reinstall forever. Record the source lock only after
  // npm ci succeeds; a changed lock or a clean node_modules forces installation.
  if (process.env.CI || installedHash !== hash) {
...
async function main() {
  ...
  await ensureBlueprintDependencies();
  const projects = await generateContent();
...
export { extractHeadingSlugs, decodeEntities };
