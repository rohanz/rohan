...
function buildMicStand() {
  const g = new THREE.Group();
  g.position.set(-0.5, 0, -3.3); // vocal position, clear floor around it
...
function buildCables() {
  const g = new THREE.Group();
  // mic (-0.5, -3.3) -> window wall
  g.add(sagCable(new THREE.Vector3(-0.42, 0, -3.22), new THREE.Vector3(-0.05, 0, -2.15), 0.015, 0.1));
