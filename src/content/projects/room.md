...
Claims are advisory. An edit inside someone else's claim raises an **interrupt**, a message that reaches the agent mid-task, but nothing is ever blocked. Developers keep their own editors, agents and Git workflow. room only adds context: it never writes a teammate's edits into your working tree, and it never commits or pushes on its own. It installs in one line per host. On Claude Code, the second line is optional: it adds a `claude-room` command (zsh) that starts a session with room's wake-up channel, so a teammate's question or a finished worker reaches your agent straight away instead of on its next turn.
...
<div class="copy-lines">
  ...
  <div class="copy-line"><pre><code>echo "alias claude-room='claude --dangerously-load-development-channels plugin:room@room'" &gt;&gt; ~/.zshrc</code></pre><button type="button" class="copy-line-btn" aria-label="Copy command"></button></div>
...
- **Waking agents.** An idle Codex agent is woken through `codex queue`. Claude Code is woken through an <span class="gloss-term" data-gloss="Model Context Protocol: a standard way to give an AI agent a set of tools it can call. room's tools (room_claim, room_read and so on) are served this way.">MCP</span> channel.
...
- **Claude Code wake-ups are opt-in.** By Claude Code's design, every plugin's channel has to be enabled per session with a research-preview flag, and on claude.ai Team and Enterprise accounts an Owner has to allow it first. A running session also only picks up plugin updates when it restarts.
