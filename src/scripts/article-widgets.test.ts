...
// Drift guard for the hand-maintained chord engine in `article-widgets.ts` (the transit theme's fork).
//
// That engine is one of three forks of the same code — the other two are `src/scripts/default/
// chord-demo.js` (classic theme) and `themes/blueprint/src/article-widgets.ts` (blueprint SPA) — all
...
// `chord-forks-parity.test.ts` (beside this file) additionally sweeps all three forks side by side.
//
