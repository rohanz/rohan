/**
 * MapView — the camera/DOM engine for the in-map site. Owns the camera state,
 * the ride/reveal choreography (GSAP timelines), platform layout (cards,
 * dividers, music-viz packing, paging), fades, and interrupt recovery
 * (watchdog / finishRide / dispose). The pure motion math it consumes
 * (trapezoid, samplers, reveal constants) lives in ./motion; the navigation
 * state machine that drives this class (URL ↔ view reconciliation) is the
 * entry, ../ride.ts.
 */
...
export class MapView {
  ...
  onSettle?: () => void;
  /** Announce hook (tab title + screen-reader live region), injected by the
   *  entry (../ride.ts owns the title/announcer state) so this class never
   *  imports the entry — no circular dependency. Fired from the settled edge
   *  below with the now-settled view. */
  onAnnounce?: (view: ViewId) => void;
  ...
  /** The shared per-frame ride mover. Every cruise (toPlatform, toMap, and the
   *  switchPlatform pass-through) drives the camera focal the same way: sample
   *  the arc-length path at the tween's progress, write x/y, derive the
   *  motion-echo direction + strength from the frame-to-frame on-screen speed
   *  (`speedPx / 22`, capped at 1 — tuned so the blur only shows at real cruise
   *  speed), apply, and remember the sample for the next frame's speed. This
   *  factory owns that closure (and its `prog`/`lastAt` state) so the three ride
   *  methods can't drift apart. `onProgress` fires AFTER the frame's state is
   *  applied, with the current progress — switchPlatform uses it to pulse Home
   *  exactly as the camera crosses the interchange. */
  private makePathMover(
    sampler: PathSampler,
    startAt: Point,
    onProgress?: (p: number) => void,
  ): { prog: { p: number }; move: () => void } {
    const prog = { p: 0 };
    let lastAt: Point = startAt;
    const move = () => {
  ...
  toPlatform(id: LineId, animate = true) {
    ...
    const { prog, move: moveSample } = this.makePathMover(sampler, startFocal);
  ...
  toMap(animate = true, onArrive?: () => void) {
    ...
    const { prog, move: moveSample } = this.makePathMover(sampler, ridePts[0]);
  ...
  switchPlatform(id: LineId) {
    ...
    // it — when prog passes the junction fraction — not on a fixed clock (the
    // once-only `junctionFired` latch lives here, outside the shared mover).
    const { prog, move: moveSample } = this.makePathMover(throughSampler, throughPts[0], (p) => {
      if (!junctionFired && p >= junctionFrac) {
        junctionFired = true;
