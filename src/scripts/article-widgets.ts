...
// live in `src/lib/visuals/`, shared with the classic and blueprint forks and
...
function sizeCanvas(canvas: HTMLCanvasElement, w: number, h: number): CanvasRenderingContext2D {
  // transit tracks the real device ratio (blueprint floors its own at 2)
...
// three themes (transit here, classic, and blueprint). It used to be a
// hand-maintained fork duplicated per theme and drifted more than once; see
// that file's header for the full story. Everything below this point is
// transit-theme DOM wiring only — piano rendering, pointer/keyboard input,
// and the readout — none of it is chord logic.
//
...
function initQlfRiskGate(node: HTMLElement) {
  ...
  // `src/lib/visuals/quant.ts` (the same rules as quantlab/risk.py). This
...
function initQuantlabAgenticVisuals() {
  if (!document.querySelector('[id^="qla2-"]')) return;
